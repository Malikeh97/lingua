...
"""
Training script for Copy Mechanism Encoder-Decoder.

Similar to train.py but uses EncDecCopyTransformer and handles
copy mechanism auxiliary outputs.
"""
...
@dataclass
class CopyTrainArgs:
    """Training arguments for copy mechanism encoder-decoder."""

    name: str = "copy_enc_dec"
    dump_dir: str = ""
    ...
    copy: CopyMechanismArgs = field(default_factory=CopyMechanismArgs)
...
def validate_train_args(args: CopyTrainArgs, vocab_size: int):
...
def should_continue_training(step: int, epoch: int, args: CopyTrainArgs) -> bool:
    if args.steps is not None:
        return step < args.steps
    return epoch < args.max_epochs


def train(args: CopyTrainArgs):
    """Main training function for copy mechanism."""
    with ExitStack() as context_stack:
        ...
        if encoder_type == EncoderType.PRETRAINED:
            ...
            model = EncDecCopyTransformer(args.model, args.copy)
        else:
        ...
        model = parallelize_model(
            ...
            args.model,
            args.distributed,
            fsdp_grouping_plan=build_copy_fsdp_grouping_plan(args.model),
            tp_parallelize=None,
        ...
        # Data loaders
        data_loader, val_loader = build_train_val_dataloaders(args.data, dp_rank, dp_degree)
...
def main():
    ...
    default_cfg = OmegaConf.structured(CopyTrainArgs())
