...
class DataPreparer:
    ...
    def _assemble_sequence(
        ...
        Args:
            segments: Tokenized segments {Q, C, A}
        ...
        for part in parts:
            if part == "S":
                ...
                if (
                    ...
                    ellipsis_ids = self.tokenizer.encode(
                        " ...", add_special_tokens=False
                    )
                ...
                prefix_ids = self._tokenize_prefix(prefix)
                if prefix_ids:
                ...
                # Add content
                part_ids = segments[part]["input_ids"]
        ...
        labels.append(self.eos_id if loss_parts else -100)
...
def get_collate_fn(pad_token_id: int, padding_side: str = "right"):
    ...
    def collate(batch):
        ...
        if "decoder_input_ids" in keys:
            result["decoder_input_ids"] = torch.tensor(
                pad_seq([x["decoder_input_ids"] for x in batch], pad_token_id)
            )
        if "labels" in keys:
...
def main():
    ...
    # Prepare data
    preparer = DataPreparer(
        tokenizer, parsed_format, args.max_length, args.dec_max_length, args.span_expr
    )
    ...
    collate_fn = get_collate_fn(tokenizer.pad_token_id, padding_side=padding_side)
    ...
    if (
        args.pretrained_weight_updating is None
        or args.pretrained_weight_updating == 0.0
    ):
        # Freeze pretrained
        ...
        optimizer_grouped_parameters = [{"params": other_params, "lr": args.lr}]
    else:
        # Scale LR for pretrained
        print(f"Scaling pretrained weights LR by {args.pretrained_weight_updating}...")

        optimizer_grouped_parameters = [
            {
                "params": pretrained_params,
                "lr": args.lr * args.pretrained_weight_updating,
            },
